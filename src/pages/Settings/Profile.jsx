...
export default function Account({ session }) {
  ...
  const { data: userData, loading: userLoading, error: userError } = useFetchUser(userID);
  ...
  async function updateProfile(event) {
    ...
    if (error2) {
      ...
      alert('Email address will only be updated once the verification email is confirmed')
      setTempEmail(email)
    }
  ...
  async function updateAvatarUrl(event, avatarUrl) {
    ...
    event.preventDefault();
    setLoading(true);
    const { error } = await supabase.from('users').upsert({ id: userID, avatar_url: avatarUrl });
    if (error) {
        ...
        <form onSubmit={updateAvatarUrl} className="form-widget">
          ...
          <div className="form-fields">
            <div className='form-field'>
              <strong><p id="full_name">{full_name || ''}</p></strong>
            </div>
            <div className='form-field'>
              <p id="email">{email || ''}</p>
            </div>
